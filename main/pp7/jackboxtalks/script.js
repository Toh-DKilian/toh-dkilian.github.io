    ...
    <div class="form-group">\r
        <div class="inputGroup">\r
            ...
            <span class="inlineSubmit">\r
                <button type="submit" class="btn btn-default inlineSubmitButton" type="button"><span class="inlineSubmitText">Îòïðàâèòü</span></button>\r
            </span>\r
...
class Ot {
    ...
    static async showAlert(e) {
        ...
        return e.customClass = {
            ...n,
            popup: "jbgModal"
        }, e.titleText = e.titleText || "Îøèáêà", Sn.fire(e)
    }
    static async showError(e) {
        ...
        return e.customClass = {
            ...i,
            popup: "jbgModal"
        }, e.titleText = e.titleText || "Îøèáêà", n && (e.imageUrl = n), Sn.fire(e)
    }
            ...
            if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
                ...
                try {
                    ...
                    console.error(i), Ot.show("alert", {
                        titleText: "Íå óäàåòñÿ ïîëó÷èòü äîñòóï ê êàìåðå",
                        text: `Ïîõîæå, ó íàñ íåò äîñòóïà ê êàìåðå òâîåãî óñòðîéñòâà. Ïåðåçàãðóçè ñòðàíèöó è ïîâòîðè ïîïûòêó èëè âûáåðè ${t}`,
                        willClose: () => {
                ...
                titleText: "Íåò äîñòóïà ê êàìåðå",
                text: `Ïîõîæå, ÷òî äîñòóï ê êàìåðå íåäîñòóïåí èç ýòîãî áðàóçåðà. Âìåñòî ýòîãî ïîïðîáóé ${t}.`,
                willClose: () => {
                    this.cameraAccessDenied()
                }
            })
...
const LC = `<canvas id="fullLayer" class="sketchpad fullLayer" width='480' height='600' style=''></canvas>`,
    ...
    <div class="canvas-container">\r
        ...
        <div id="post-sketchpad" class="post-sketchpad">\r
            <div id="submit">\r
                <button id='submitdrawing' class="button submitDrawing">Îòïðàâèòü</button><br/>\r
            </div>\r
            <button id='censorOptions' class='button'>Ïàðàìåòðû öåíçóðû</button>\r
...
`,
    NC = Tn.extend({
        defaults: {
            ...
            debug: !1,
            strings: {
                drawing_empty: "Íàäî ÷òî-òî íàðèñîâàòü!",
                submit: "Îòïðàâèòü",
                ERROR_REJECTED_OBJECT: "Ýòî çàïðåùåíî, íàðèñóé/ââåäè ÷òî-íèáóäü åù¸!"
            }
        }
    }),
    Vn = ft.View.extend({
        ...
        bindings: {
            ...
            ".submitDrawing": {
                ...
                updateView: !0,
                onGet(t) {
                    return t[0] || t[1] ? !1 : t[2] === void 0 ? "" : t[2].submit || "Îòïðàâèòü"
                }
            }
        },
        initialize() {
...
`,
    $C = Tn.extend({
        defaults: {
            ...
            className: "",
            inlineSubmit: !1,
            inlineSubmitText: "Îòïðàâèòü",
            error: "",
            strings: {
                ERROR_NOTHING_ENTERED: "Òåáå íóæíî ÷òî-òî íàïèñàòü!",
                ERROR_REJECTED_TEXT: "Ìíå êàæåòñÿ, ÷òî òåáå íåëüçÿ òàêîå ïèñàòü. Ïîæàëóéñòà, áóäü óâàæèòåëåí ê äðóãèì èãðîêàì."
            }
        }
    }),
    jC = ft.View.extend({
...
ft.View.extend({
    ...
    showTwitchBroadcasterDialog(t) {
        ...
        e += "<div class='success'>Òû óñïåøíî ïîäêëþ÷èë ñâîþ ó÷¸òíóþ çàïèñü ê ðàñøèðåíèþ Jackbox Audience Kit äëÿ Twitch.</div>", this.lacksAudience ? e += "<div class='warning'>Â ÝÒÎÉ ÈÃÐÅ ÍÅÒ ÇÐÈÒÅËÅÉ</div>" : this.client.roomInfo.audienceEnabled || (e += "<div class='warning'>Â ÝÒÎÉ ÈÃÐÅ ÂÛÊËÞ×ÅÍÛ ÇÐÈÒÅËÈ</div>"), Ot.show("custom", {
            html: e,
            position: "bottom",
            timer: t,
...
const zC = `<div id="controller" class="state-controller controller-content">
    ...
    <div id="artifactId" class="artifactContainer text">
        <a id="artifactLink" aria-label="Ïîñìîòðåòü ãàëåðåþ" class="artifactLink" target="_blank">
    ...
    UC = Tn.extend({
        defaults: {
            ...
            isLocal: !1,
            strings: {
				wait: "Ñÿäü ïîóäîáíåå è ðàññëàáüñÿ!",
				vip_waiting: "Æä¸ì, êîãäà âñå èãðîêè ïðèñîåäèíÿòñÿ",
				vip_canStart: "Íàæìè ýòó êíîïêó, êîãäà âñå ïðèñîåäèíÿòñÿ",
				vip_cancel: "Íàæìè ýòó êíîïêó, ÷òîáû îòìåíèòü çàïóñê èãðû",
				vip_postgame: "×òî òåïåðü?",
                vip_episodes_menu: "Ìåíþ ýïèçîäîâ",
                vip_episodes_unload: "Âûãðóçèòü ýïèçîä",
                vip_episodes_report: "Ïîæàëîâàòüñÿ íà ýïèçîä",
                vip_episodes_warning: "Ïðåäóïðåæäåíèå: ïîëüçîâàòåëüñêèé êîíòåíò íå ïðîâåðÿåòñÿ",
                vip_episodes_load: "Çàãðóçèòü ýïèçîä ïî id:",
                vip_episodes_select: "Èëè âûáðàòü ýïèçîä èç ñïèñêà:",
                vip_episodes_back: "Íàçàä",
                vip_episodes_submit: "Ïîäòâåðäèòü",
                vip_episodes_view_author: "Ïîêàçàòü àâòîðà",
				button_start: "Âñå çäåñü",
				button_cancel: "Îòìåíà",
				button_changename: "Èçìåíèòü èìÿ",
				button_sameplayers: "Òå æå èãðîêè",
				button_newplayers: "Íîâûå èãðîêè",
				prompt_entername: "Ââåäè ñâî¸ èìÿ",
				prompt_choosecharacter: "Âûáåðè ñâîåãî ïåðñîíàæà",
				button_censorOptions: "Ïàðàìåòðû öåíçóðû",
                censor_prompt: ""
            }
        }
    }),
            ...
            if (i !== "back" && i !== "censorConfirm" && i !== "activateContentId")
                if (i === "changeName") try {
                        const a = await Ot.show("custom", {
                            ...
                            inputValidator: f => f ? f.length > 12 ? "Ìàêñèìóì 12 ñèìâîëîâ" : null : "Òåáå íóæíî ÷òî-íèáóäü íàïèñàòü!"
                        });
                        if (a.dismiss) return;
                        this.triggerMethod("client:message", {
                            name: a.value
...
<div id="artifactId" class="artifactContainer text">
    <a id="artifactLink" aria-label="Ïîñåòèòü ãàëåðåþ" class="artifactLink" target="_blank">
        <button id="artifactButton" class="artifactButton"></button>
    </a>
</div>
`,
    ...
    zs = {
        en: {
            ...
            LANGUAGE: "ßçûê",
            LOGIN: "Ëîãèí",
            STRING_ERROR_SERVER_ROOM_DISCONNECTED: "Îòêëþ÷¸í",
            STRING_ERROR_SERVER_ROOM_DESTROYED: "Ñïàñèáî çà èãðó!"
        },
        fr: {
    ...
    JC = Tn.extend({
        defaults: {
            ...
            countGroupName: null,
            strings: {
                your_choice: "Ñïàñèáî. Ñâîé âûáîð: ",
                censor_prompt: "Óáðàòü ýòî?",
                censor_confirm: "Äà, óáåðèòå!",
                censor_cancel: "Íåò!"
            }
        }
    }),
...
const _x = `<div id="controller" class="state-controller controller-content">\r
    ...
    Sx = Tn.extend({
        defaults: {
            ...
            announcePrompt: !1,
            strings: {
                your_choice: "Ñïàñèáî. Òâîé âûáîð: ",
                censor_prompt: "Óáðàòü ýòî?",
                censor_confirm: "Äà, óáåðèòå!",
                censor_cancel: "Íåò!"
            }
    ...
    Rx = Tn.extend({
        defaults: {
            ...
            validActions: [],
            strings: {
				tos_warning: "Äåëÿñü êîíòåíòîì, òû ñîãëàøàåøüñÿ ñ Óñëîâèÿìè èñïîëüçîâàíèÿ",
				tos_warning_agree: "ñîãëàñèòüñÿ è ïîäåëèòüñÿ",
				tos_warning_back: "âåðíóòüñÿ",
				create_new_episode: "ñîçäàòü íîâûé ýïèçîä",
				create_new_name_prompt: "ïåðâî-íàïåðâî ââåäè íàçâàíèå ýïèçîäà è íàæìè Ñîçäàòü.",
				create_new_button: "ñîçäàòü",
				button_back_to_episodes: "âåðíóòüñÿ ê ýïèçîäàì",
				button_back_to_menu: "âåðíóòüñÿ â ìåíþ",
				previous_episodes: "ïðåäûäóùèå ýïèçîäû:",
				toggle_prompts_prompt: "íàæìè, ÷òîáû ïîêàçàòü/ñêðûòü âîïðîñû",
				button_close: "çàêðûòü",
				button_done: "ãîòîâî",
				button_add: "äîáàâèòü âîïðîñ",
				input_placeholder: "ââåäè âîïðîñ",
				label_hidden: "ñêðûòî",
				button_edit: "ðåäàêòèðîâàòü",
				button_save: "ñîõðàíèòü",
				button_publish: "îïóáëèêîâàòü",
				button_play: "èãðàòü",
				button_delete: "óäàëèòü",
				delete_warning: "Òû òî÷íî õî÷åøü óäàëèòü ýòîò ýïèçîä?",
				delete_warning_confirm: "Äà",
				delete_warning_cancel: "Íåò"
}
        }
    }),
    Ix = ft.View.extend({
...
const Bx = ft.View.extend({
    ...
    showTwitchBroadcasterDialog(t) {
        ...
        e += `
            <div class='success'>
                Òû óñïåøíî ïîäêëþ÷èë ñâîþ ó÷¸òíóþ çàïèñü ê ðàñøèðåíèþ Jackbox Audience Kit äëÿ Twitch.
            </div>`, this.lacksAudience ? e += "<div class='warning'>Â ÝÒÎÉ ÈÃÐÅ ÍÅÒ ÇÐÈÒÅËÅÉ</div>" : this.client.roomInfo.audienceEnabled || (e += "<div class='warning'>Â ÝÒÎÉ ÈÃÐÅ ÂÛÊËÞ×ÅÍÛ ÇÐÈÒÅËÈ</div>"), Ot.show("custom", {
            html: e,
            position: "bottom",
            timer: t,
            backdrop: "transparent",
...
`,
    ...
    <div class="canvas-container">
        ...
        <div id="post-sketchpad" class="post-sketchpad">
            <div id="submit">
                <button id='submitdrawing' class="button submitDrawing">Ïîäòâåðäèòü</button><br/>
            </div>
            <button id='censorOptions' class='button'>Ïàðàìåòðû öåíçóðû</button>
    ...
    Qx = Vn.extend({
        ...
        sendInput(t) {
            if (this.sketchpadComponent.getLines().length === 0) {
                Ot.show(Error("Òû äîëæåí ÷òî-íèáóäü íàðèñîâàòü!"));
                return
            }
...
<div class="cameraControls">
    ...
    <div class="buttons post">
        ...
        <button id="confirmButton" class="button confirmButton" aria-label="Confirm">ÎÑÒÀÂÈÒÜ</button>
    </div>
</div>
<div style="display:none;">
    ...
    <div id="slides">
        ...
        <div class="add">
            <button>Äîáàâèòü ñëàéä</button>
        </div>
    </div>    
</div>
<div class="submitRegion footer">
    <button class="submit">ÎÒÏÐÀÂÈÒÜ</button>
</div>`,
    c1 = Tn.extend({
        defaults: {
            prompt: {},
